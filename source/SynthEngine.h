...
struct BlacksideOscillator
{
    ...
    void setWavePos(float value)    { wavePos = juce::jlimit(0.0f, 1.0f, value); }
    void setWarp(float value)       { warp = juce::jlimit(0.0f, 1.0f, value); }

    float process(float phaseOffset = 0.0f)
    {
        const float step = frequency / static_cast<float>(sampleRate);
        phase += step;
        while (phase >= 1.0f)
            phase -= 1.0f;
        ...
        const float square = (p < (0.5f + warp * 0.22f)) ? 1.0f : -1.0f;
        const float metal  = std::sin((p + saw * warp * 0.18f) * juce::MathConstants<float>::twoPi * (2.0f + warp * 9.0f));
        const float vowel  = std::tanh((saw * 0.55f + sine * 0.45f) * (1.5f + warp * 6.0f));

        std::array<float, 6> waves { sine, saw, tri, square, metal, vowel };
        const float sel = juce::jlimit(0.0f, 0.999f, wavePos) * static_cast<float>(waves.size() - 1);
        const int i0 = static_cast<int>(sel);
        const int i1 = juce::jmin(static_cast<int>(waves.size()) - 1, i0 + 1);
        const float frac = sel - static_cast<float>(i0);

        float out = juce::jmap(frac, waves[static_cast<size_t>(i0)], waves[static_cast<size_t>(i1)]);
...
struct BlacksideVoice
{
    ...
    void start(int note, float vel, bool retriggerEnvelope)
    {
        midiNote  = note;
        velocity  = vel;
        targetHz  = static_cast<float>(juce::MidiMessage::getMidiNoteInHertz(note));
    ...
    float nextSample(
        ...
        float macro1, float macro2, float macro3, float macro4,
        ...
        float subOctaveMix, float outputClip)
    {
        ...
        const float glideCoeff = (glideMs <= 0.0f)
            ? 1.0f
            : (1.0f - std::exp(-1.0f / (0.001f * glideMs * static_cast<float>(sampleRate))));
        currentHz += (targetHz - currentHz) * glideCoeff;

        lfoPhase1 += lfoRate / static_cast<float>(sampleRate);
        lfoPhase2 += (lfoRate * 0.31f + 0.17f) / static_cast<float>(sampleRate);
        ...
        const float lfo2 = std::sin(lfoPhase2 * juce::MathConstants<float>::twoPi) * (lfoAmount * 0.5f + macro3 * 0.15f);
        ...
        oscC.setFrequency(currentHz * (0.5f + macro2 * 0.35f + macro3 * 0.10f));
        ...
        oscC.setWavePos(juce::jlimit(0.0f, 1.0f, oscCWave + hornFormant * 0.2f + macro3 * 0.10f));
        harmonyOsc.setWavePos(juce::jlimit(0.0f, 1.0f, oscBWave + 0.2f + macro3 * 0.08f));
        subOsc.setWavePos(0.12f);
        subOctOsc.setWavePos(0.08f);
        noiseOsc.setWavePos(0.8f);

        oscA.setWarp(juce::jlimit(0.0f, 1.0f, oscAWarp + macro1 * 0.22f + screechFm * 0.15f));
        oscB.setWarp(juce::jlimit(0.0f, 1.0f, oscBWarp + macro1 * 0.16f + screechFm * 0.12f));
        oscC.setWarp(juce::jlimit(0.0f, 1.0f, oscCWarp + macro2 * 0.14f + hornBody * 0.18f));
        harmonyOsc.setWarp(juce::jlimit(0.0f, 1.0f, 0.12f + harmMix * 0.25f + macro3 * 0.18f));
        subOsc.setWarp(juce::jlimit(0.0f, 1.0f, subDrive * 0.35f + macro4 * 0.10f));
        subOctOsc.setWarp(juce::jlimit(0.0f, 1.0f, subDrive * 0.28f));
        noiseOsc.setWarp(juce::jlimit(0.0f, 1.0f, screechDrive * 0.7f + macro4 * 0.15f));
        ...
        const float sub = subOsc.process() * subLevel;
        const float sub2 = subOctOsc.process() * subOctaveMix * 0.5f;
        const float noise = noiseOsc.process() * noiseLevel * (0.12f + screechDrive * 0.25f + macro3 * 0.10f);

        float x = a + b + c + h + noise;
        x += std::tanh((sub + sub2) * (1.0f + subDrive * 5.0f + macro4 * 1.25f));

        const float dynamicCutoff1 = juce::jlimit(30.0f, 18000.0f,
            cutoff1 * (1.0f + env * envAmount + lfo1 * 0.8f + macro2 * 0.25f + macro4 * 0.35f));
        const float dynamicCutoff2 = juce::jlimit(30.0f, 18000.0f,
            cutoff2 * (1.0f + lfo2 * 0.6f + hornFormant * 0.5f + macro1 * 0.2f + macro4 * 0.20f));

        filter1.setCutoffFrequency(dynamicCutoff1);
        filter1.setResonance(juce::jlimit(0.1f, 1.4f, res1 + reeseWidth * 0.15f + macro4 * 0.12f));
        filter2.setCutoffFrequency(dynamicCutoff2);
        filter2.setResonance(juce::jlimit(0.1f, 1.2f, res2 + hornBody * 0.12f + macro3 * 0.08f));
        ladder.setCutoffFrequencyHz(juce::jlimit(40.0f, 16000.0f, dynamicCutoff1 * (0.7f + hornBody * 0.4f)));
        ladder.setResonance(juce::jlimit(0.1f, 0.95f, 0.2f + screechFm * 0.35f + macro3 * 0.10f));

        x = std::tanh(x * (1.0f + drive1 * 5.5f + screechDrive * 2.5f + macro3 * 1.8f));
        x = filter1.processSample(0, x);
        x += filter2.processSample(0, x) * (0.28f + hornFormant * 0.24f);
        x = 0.72f * x + 0.28f * ladder.processSample(x);

        const float dirty = std::tanh(x * (1.0f + distDrive * 7.0f + screechDrive * 5.0f + macro3 * 2.5f));
        x = juce::jmap(distMix, x, dirty);

        x = 0.86f * x + 0.14f * last;
        last = x;

        const float hp = x - dcPrevIn + 0.995f * dcPrevOut;
        dcPrevIn = x;
        dcPrevOut = hp;
        x = hp;
