...
juce::AudioProcessorValueTreeState::ParameterLayout BadlineDnBAudioProcessor::createParameters()
{
    ...
    p.push_back(std::make_unique<APC>("play_mode", "Play Mode", juce::StringArray { "Mono", "Legato", "Poly" }, 0));
...
void BadlineDnBAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    const auto playMode = static_cast<PlayMode>(static_cast<int>(apvts.getRawParameterValue("play_mode")->load()));
    ...
    const float macro3      = apvts.getRawParameterValue("macro3")->load();
    const float macro4      = apvts.getRawParameterValue("macro4")->load();
    ...
    for (int i = 0; i < buffer.getNumSamples(); ++i)
    {
        ...
        subPeak = juce::jmax(subPeak, std::abs(s) * (subLevel + subOctave * 0.5f));
        pushScopeSample(s);
...
void BadlineDnBAudioProcessor::copyScopeData(std::array<float, 512>& dest) const noexcept
{
    ...
    const int size = static_cast<int>(scopeRing.size());
    for (int i = 0; i < static_cast<int>(dest.size()); ++i)
    {
        const int index = (write - static_cast<int>(dest.size()) + i + size) % size;
        dest[static_cast<size_t>(i)] = scopeRing[static_cast<size_t>(index)];
    }
}
...
void BadlineDnBAudioProcessor::handleNoteOn(int note, float velocity, PlayMode mode)
{
    ...
    noteVelocities[static_cast<size_t>(note)] = velocity;
    ...
    {
        const auto priority = static_cast<NotePriority>(static_cast<int>(apvts.getRawParameterValue("note_priority")->load()));
...
void BadlineDnBAudioProcessor::handleNoteOff(int note, PlayMode mode)
{
    ...
    if (sustainPedalDown)
    {
        ...
        if (mode == PlayMode::poly)
        {
            ...
            if (idx >= 0)
                voices[static_cast<size_t>(idx)].heldBySustain = true;
        }
        return;
    }
...
void BadlineDnBAudioProcessor::handleSustainPedal(bool down, PlayMode mode)
{
    ...
    {
        if (sustained[static_cast<size_t>(note)] && ! keyDown[static_cast<size_t>(note)])
        {
            sustained[static_cast<size_t>(note)] = false;
            if (mode == PlayMode::poly)
                releasePolyNote(note);
...
void BadlineDnBAudioProcessor::retriggerMonoFromStack()
{
    ...
    if (sustainPedalDown && sustained[static_cast<size_t>(nextNote)])
        return;

    const float nextVelocity = juce::jlimit(0.0f, 1.0f, noteVelocities[static_cast<size_t>(nextNote)]);
    if (voices[0].active)
        voices[0].legatoTo(nextNote, nextVelocity > 0.0f ? nextVelocity : 1.0f);
    else
        voices[0].start(nextNote, nextVelocity > 0.0f ? nextVelocity : 1.0f, true);
}
...
{
    for (int i = 0; i < maxVoices; ++i)
        if (voices[static_cast<size_t>(i)].active && voices[static_cast<size_t>(i)].midiNote == note)
            return i;
    return -1;
}
...
int BadlineDnBAudioProcessor::stealVoice(NotePriority priority) const
{
    ...
    if (priority == NotePriority::low)
    {
        ...
        for (int i = 0; i < maxVoices; ++i)
        {
            const auto note = voices[static_cast<size_t>(i)].midiNote;
            if (voices[static_cast<size_t>(i)].active && note < lowest)
            {
                lowest = note;
                candidate = i;
            }
    ...
    else if (priority == NotePriority::high)
    {
        ...
        {
            const auto note = voices[static_cast<size_t>(i)].midiNote;
            if (voices[static_cast<size_t>(i)].active && note > highest)
            {
                highest = note;
                candidate = i;
            }
...
void BadlineDnBAudioProcessor::startPolyVoice(int note, float velocity, NotePriority priority)
{
    ...
    voices[static_cast<size_t>(idx)].start(note, velocity, true);
}
...
void BadlineDnBAudioProcessor::releasePolyNote(int note)
{
    const int idx = findVoiceForNote(note);
    if (idx >= 0)
        voices[static_cast<size_t>(idx)].stop();
}
...
void BadlineDnBAudioProcessor::pushScopeSample(float s) noexcept
{
    ...
    scopeRing[static_cast<size_t>(pos % size)] = s;
}

void BadlineDnBAudioProcessor::pushAnalyzerSample(float s) noexcept
{
    if (fftFifoIndex < static_cast<int>(fftFifo.size()))
...
void BadlineDnBAudioProcessor::computeAnalyzerFrame() noexcept
{
    ...
    {
        const size_t src = juce::jmin(static_cast<size_t>(511), i * static_cast<size_t>(4));
